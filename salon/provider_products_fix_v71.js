// FiestaControl V71 - Mis productos sin bloquear la navegación
(function(){
'use strict';
...
function isProviderPortal(){
  ...
  return side.includes('portal proveedor');
}
...
function currentProvider(){
  ...
  const ids=[s.marketSupplierId,s.providerId,s.supplierId,s.userId,s.id]
...
function pName(p){
  return String(p?.fantasyName||p?.businessName||p?.business||p?.name||p?.email||'Proveedor');
}
...
function openProduct(productId=''){
    ...
    <form id="fc-v71-product-form">
      <div class="form-grid">
        ...
        <div class="field span2">
          ...
          <input name="name" required value="${esc2(old?.name||'')}">
        </div>

        <div class="field">
          <label>Categoría</label>
          <input name="category" required value="${esc2(old?.category||'')}">
        </div>
        ...
        <div class="field span2">
          ...
          <textarea name="description" required>${esc2(old?.description||'')}</textarea>
        </div>
  ...
  document.querySelector('#fc-v71-product-form').onsubmit=ev=>{
    ev.preventDefault();
...
function renderProducts(){
                ...
                <tr>
                  <td>
                    ${x.photo
                      ...
                      : `<div style="width:58px;height:58px;border-radius:9px;background:#f1f2f6;display:flex;align-items:center;justify-content:center">📦</div>`
                    }
                  </td>
  ...
  document.querySelector('#fc-v71-add').onclick=()=>openProduct();
...
// IMPORTANTE V71:
// No hay MutationObserver ni setInterval que vuelvan a abrir Mis productos.
// Se renderiza SOLO cuando el usuario hace click en "Mis productos".
// Por eso Inicio, Salones, Comunidad, Pedidos y Finanzas vuelven a navegar normalmente.
document.addEventListener('click',ev=>{
  ...
  if(text==='mis productos'){
    ev.preventDefault();
    ev.stopPropagation();
    ev.stopImmediatePropagation();

    // Permitimos que la clase activa del menú quede visualmente correcta.
    document.querySelectorAll('.sidebar .active,.nav .active').forEach(x=>x.classList.remove('active'));
  ...
  // Si el usuario va a otra sección, V71 NO vuelve a renderizar productos.
  if(['inicio','salones','comunidad','pedidos','finanzas'].includes(text)){
    const content=document.querySelector('#content');
    if(content)content.removeAttribute('data-v71-products');
  }
},true);

setTimeout(removeOldMenuItem,200);
setInterval(removeOldMenuItem,1500);

window.FCProviderProductsV71={render:renderProducts,open:openProduct};
})();
