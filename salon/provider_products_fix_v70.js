// FiestaControl V70 - fix directo de "Mis productos" en Portal proveedor
(function(){
'use strict';
...
function currentProvider(){
  ...
  return list.find(x=>
    (x.email&&side.includes(norm(x.email))) ||
    (x.fantasyName&&side.includes(norm(x.fantasyName))) ||
    (x.businessName&&side.includes(norm(x.businessName))) ||
    (x.name&&side.includes(norm(x.name)))
  )||null;
}
...
function openProduct(productId=''){
  ...
  document.querySelector('#fc-v70-product-form').onsubmit=ev=>{
    ev.preventDefault();
...
function renderProducts(){
  ...
  document.querySelector('#fc-v70-add').onclick=()=>openProduct();
...
function isMisProductos(){
  const active=[...document.querySelectorAll('.sidebar .active,.nav .active')]
    .some(x=>norm(x.textContent)==='mis productos');
  const title=norm(document.querySelector('#title')?.textContent||'');
  return active||title==='mis productos';
}
...
function clean(){
  ...
  if(!isMisProductos()){
    [...document.querySelectorAll('button,a,[role="button"]')].forEach(el=>{
      ...
      if(t.includes('agregar producto')||t==='+ producto')el.remove();
    });
  }
}
...
document.addEventListener('click',ev=>{
  ...
  if(norm(node.textContent)==='mis productos'){
    ev.preventDefault();
    ev.stopPropagation();
...
const obs=new MutationObserver(()=>{
  clean();
  if(isProviderPortal()&&isMisProductos()&&!document.querySelector('#fc-v70-products')){
    setTimeout(renderProducts,0);
  }
});
...
setTimeout(()=>{
  clean();
  if(isProviderPortal()&&isMisProductos())renderProducts();
},250);

window.FCProviderProductsV70={render:renderProducts,open:openProduct};
})();
