...
function render(){
  ...
  host.className='fc-v84-admin-list';
...
function badge(){
  ...
  if(n){
    ...
    b.className='fc-v84-community-badge';
...
// RESET GLOBAL V83:
// Una sola vez deja en cero los mensajes de Comunidad para TODOS los usuarios.
// Después, los mensajes nuevos vuelven a funcionar normalmente.
async function resetOnce(src){
  if(src?.communityResetV83===true){
    data.communityResetV83=true;
    return false;
  }
  ...
  data.communityResetV83=true;
  data.communityResetAtV83=new Date().toISOString();

  save();
  // Confirmamos el reset contra el servidor antes de volver a pintar.
...
window.syncCommunityMessagesV84=sync;
})();
